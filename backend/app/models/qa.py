...
class QAUpvote(Base):
    """Tracks unique user upvotes per QA answer to enforce 1 upvote per user."""
    __tablename__ = "qa_upvotes"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    answer_id = Column(UUID(as_uuid=True), ForeignKey("qa_answers.id"), nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)

    __table_args__ = (
        UniqueConstraint("user_id", "answer_id", name="uq_user_answer_upvote"),
    )
